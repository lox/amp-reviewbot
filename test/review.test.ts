...
describe("buildReviewPrompt", () => {
  it("embeds one self-contained methodology without skill metadata", () => {
    ...
    assert.match(prompt, /Report only material issues that pass all four checks/)
    assert.match(prompt, /stated intent, changed behavior, a supported contract/)
    assert.match(prompt, /Concrete harm.*realistic conditions/)
    assert.match(prompt, /Maintenance burden counts as concrete harm only when it is specific/)
    assert.match(prompt, /defence in depth without a credible present threat/)
    assert.match(prompt, /Objective improvement alone is not a reason to ask for work/)
