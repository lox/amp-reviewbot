...
Three prompt-wording attempts at severity calibration (tightened high, scope gate, severity calibration gate) each lowered blocking detection by one to three calls while leaving the persistent wrong blocks in place. The reviewer's severity judgement on #2759, #3820, and #3931 has not moved under any wording, and the severity re-pass below (a change of mechanism, not wording) did not move it either. On corrected labels the production prompt blocks 25/29 development and 17/18 holdout bugs and wrongly blocks 5/22 and 3/10 (51/60 and 28/30 versions completed; the rest are excluded reviews). Severity-suppression experiments are closed: every one cost real blocks without removing the persistent wrong blocks. `current` stays. Effort moves to the four development misses (audit the saved artifacts offline first: detection, calibration, changed-line filtering, or unstable sampling) and to monitoring block validity on real pull requests. A multi-sample majority on the block decision is only worth an A/B if that audit shows sampling instability behind at least three of the misses.
