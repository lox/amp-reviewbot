...
Add a holdout-only tranche of 10–12 human-reviewed post-v4 pull requests, using the same protocol as the existing human-review group: a non-author reviewer requested a concrete change, independent evidence confirms the issue, and the baseline is checked directly. Aim for roughly half blocking and half advisory-only (a confirmed issue below `high`), because the wrong-block pattern (recoverable or development-only problems rated high) is where a code-base shift is most likely to show, and every persistent wrong block so far has been on an advisory-only or clean version. The tranche has no clean versions: the pack schema requires a human-review example to carry a known issue, so clean versions exist only as the baselines of synthetic pairs. Skip v4 synthetic mutants at first, since the human-reviewed tranche tells us whether the shift matters before we spend authoring effort on pairs; when pairs are added, their `clean-control` baselines supply the post-v4 clean versions.
...
- The tranche goes to the holdout, not development. It is a check that the production prompt has not drifted against current code, and holdout results are final. It stays held back: if a v4-specific weakness turns up that needs iteration, author a separate post-v4 development tranche to iterate on, so the holdout still has unseen post-v4 examples to validate the resulting prompt against.
...
- `fast-v3`, if one is ever needed, is built from that post-v4 development tranche, never from the holdout tranche: selecting a tuning set from held-back examples whose `current` outcomes are already known would leak them into the inner loop. As with `fast-v2`, it is selected from settled calls after the development tranche has been scored once under `current`; adding unscored versions to a fast set would make an A/B result depend on which prompt happened to see them first.
- Scoring the tranche means running only the new versions. `run --split holdout` would buy the 30 existing holdout reviews again, and `run` selects by split and version kind, not by example, so the first step is a small framework change: a `--set NAME` option on `run` that restricts the holdout split to a frozen set's members. Through that path the tranche gets the standard protocol (three repeats, finding-match judging, `finish`) and a score that can sit beside the v3 holdout numbers in [eval-experiments.md](../eval-experiments.md), recorded separately rather than merged into them. Until then, `ab PACK SET current current --split holdout` (the flag is required; `ab` defaults to the development split and rejects a set whose members belong to the other) is only a preliminary screen, as `gap-v1` was: one sample per version, binary calls only, and a block for an unrelated finding counts the same as a block for the recorded bug. Record a screen as a screen, not as the tranche's score. Either run needs the same explicit confirmation as any other reviewer run.
