...
Source setup is a separate first turn. The reviewer is asked only to run the complete setup block as its first and only tool call; any failed command stops the block. The runner checks that turn before it sends the normal review prompt to the same thread. If setup changed, failed, or used another tool first, the sample stops without a review. A successful review then uses only that prepared copy of the target repository. It must not inspect the target pull request through GitHub pages or APIs, nor clone, fetch, or inspect another copy of the target repository. Public documentation, package registries, dependencies, other repositories, and delegated research that follows the same rule are allowed.

The saved trace is checked for four things: the setup turn used the exact command and completed successfully before the review began; the review used the chosen Amp mode and did not explicitly move shell commands outside its prepared workspace; research tools did not name the target pull request or repository; and the prepared repository did not run `git fetch` or `git pull`. Amp reports the trusted machine's path during startup and the mounted orb path in tool calls, so the first shell command establishes the workspace used by later checks. A breach makes the run invalid for comparison.
