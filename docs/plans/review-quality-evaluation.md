...
- The first tranche goes to development, not the holdout. Its purpose is to find out whether the production prompt has drifted against current code, and answering that means scoring `current` on it and reading the results; examples whose outcomes have been read and used to motivate a prompt change are development examples by definition, whatever split they were filed under. Authoring separate development examples afterwards would not make the inspected ones unseen again. A post-v4 holdout tranche is authored separately and only if a candidate prompt comes out of that iteration; it is held back unscored (not even a `current`-vs-`current` screen) until the candidate is selected, and its single A/B run is final.
...
- `fast-v3`, if one is ever needed, is built from the post-v4 development tranche, never from a post-v4 holdout tranche: selecting a tuning set from held-back examples would leak them into the inner loop. As with `fast-v2`, it is selected from settled calls after the development tranche has been scored once under `current`; adding unscored versions to a fast set would make an A/B result depend on which prompt happened to see them first.
- Scoring the tranche means running only the new versions. `run --split development` would buy the 53 existing development reviews again, and `run` selects by split and version kind, not by example, so the first step is a small framework change: a `--set NAME` option on `run` that restricts a split to a frozen set's members. Through that path the tranche gets the standard protocol (three repeats, finding-match judging, `finish`) and a score that can sit beside the v3 numbers in [eval-experiments.md](../eval-experiments.md), recorded separately rather than merged into them. Until then, `ab PACK SET current current` is only a preliminary screen, as `gap-v1` was: one sample per version, binary calls only, and a block for an unrelated finding counts the same as a block for the recorded bug. Record a screen as a screen, not as the tranche's score. Either run needs the same explicit confirmation as any other reviewer run.
