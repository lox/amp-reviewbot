...
- Scoring the tranche is one paid run over the new versions only. `run --split holdout` would buy the 30 existing holdout reviews again, so freeze the tranche as a set and run `ab PACK SET current current --split holdout`, as `gap-v1` did for its development members. `ab` defaults to the development split and rejects a set whose members belong to the other split, so the flag is required here. It needs the same explicit confirmation as any other reviewer run and is recorded in [eval-experiments.md](../eval-experiments.md) beside the v3 holdout numbers, not merged into them.
